...
test_dir = f'./data/%s/test/' % name
Data_dir = f'./data/%s/' % name
...
enhancers_tra = open(train_dir + '%s_enhancer.fasta' % name, 'r').read().splitlines()
promoters_tra = open(train_dir + '%s_promoter.fasta' % name, 'r').read().splitlines()
...
# enhancers_tes = open(test_dir + '%s_enhancer_test.csv' % name, 'r').read().splitlines()[1::2]
# promoters_tes = open(test_dir + '%s_promoter_test.csv' % name, 'r').read().splitlines()[1::2]
...
np.savez(Data_dir + f'%s_train_new.npz' % name, X_en_tra=X_en_tra, X_pr_tra=X_pr_tra, y_tra=y_tra)
# np.savez(Data_dir + 'im_%s_train.npz' % name, X_en_tra=X_en_imtra, X_pr_tra=X_pr_imtra, y_tra=y_imtra)
np.savez(Data_dir + f'%s_test_new.npz' % name, X_en_tes=X_en_tes, X_pr_tes=X_pr_tes, y_tes=y_tes)
